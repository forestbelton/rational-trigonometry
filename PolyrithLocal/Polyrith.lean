...
Authors: Dhruv Bhatia, Eric Wieser, Mario Carneiro, Thomas Zhu, Forest Belton
-/
-- Adapted from Mathlib's removed `polyrith` tactic to call a local Sage installation
-- instead of the (defunct) SageCell web API.
...
partial def parse {u : Level} {α : Q(Type u)} (sα : Q(CommSemiring $α))
    (c : Cache sα) (e : Q($α)) : AtomM Poly := do
  let els := do
...
def sageHelperFunctions : String :=
  include_str "polyrith_sage_helper.py"
...
/--
This function runs a local Sage installation on the generated query and parses the
output as a `SageResult`. The `sage` binary must be available on `PATH`.

The query is a Sage program (the helper functions from `polyrith_sage_helper.py` plus a
call to `main`) whose `main` prints the certificate as JSON (in the `SageCoeffAndPower`
format) to stdout. Unlike the old SageCell web API there is no JSON envelope around the
result, so stdout is parsed directly.
-/
def runSage (trace : Bool) (α : Expr) (atoms : Nat) (hyps : Array (Source × Poly)) (target : Poly) :
  ...
  -- Write the query to a temporary `.sage` file. The helper uses Sage preparser syntax
  -- (e.g. `^` for exponentiation and `1/p`), so it must be run as a `.sage` file rather
  -- than via `sage -python`, which would disable preparsing.
  let tmpDir := (← IO.getEnv "TMPDIR").getD "/tmp"
  let scriptPath := System.FilePath.mk s!"{tmpDir}/polyrith_{hash query}.sage"
  IO.FS.writeFile scriptPath query
  -- run local Sage, then clean up the temporary script. The binary defaults to `sage`
  -- on `PATH`, but can be overridden with the `POLYRITH_SAGE` environment variable
  -- (useful when `sage` is only a shell alias and not on `PATH`).
  let sageCmd := (← IO.getEnv "POLYRITH_SAGE").getD "sage"
  let out ← IO.Process.output { cmd := sageCmd, args := #[scriptPath.toString] }
  try IO.FS.removeFile scriptPath catch _ => pure ()
  if out.exitCode != 0 then
    return .error
      { name := "SageError"
        value := s!"sage exited with code {out.exitCode}:\n{out.stderr}" }
  -- `main` prints a bare `SageCoeffAndPower` JSON to stdout (no SageCell envelope).
  let parsed : Except String SageCoeffAndPower := Json.parse out.stdout >>= fromJson?
  match parsed with
  | .ok coeffAndPower => return .ok { data := some coeffAndPower }
  | .error e => IO.throwServerError <|
      s!"Could not parse Sage output (error: {e})\n\
        Sage stdout:\n{out.stdout}\nSage stderr:\n{out.stderr}"
